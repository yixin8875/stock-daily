...
import { Card, Table, Button, Space, Tag, Switch, Modal, Form, Select, InputNumber, message, Popconfirm, Empty } from 'antd'
import { BarChartOutlined, PlusOutlined, DeleteOutlined, ReloadOutlined } from '@ant-design/icons'
...
const VolumeAlert: React.FC = () => {
  ...
  const fetchAlerts = async () => {
    ...
    try {
      ...
      const filtered = (res.data.data || []).filter(a =>
        ['VOLUME_SURGE', 'VOLUME_SHRINK'].includes(a.alertType)
      )
      setAlerts(filtered)
    } catch {
  ...
  const columns = [
    ...
    {
      title: '阈值', dataIndex: 'volumeThreshold', key: 'volumeThreshold',
      render: (v: number) => v ? `${v}%` : '-'
    },
    {
      title: '状态', key: 'status',
  ...
  return (
    <Card
      title={<><BarChartOutlined style={{ marginRight: 8 }} />成交量异动提醒</>}
    ...
    >
      {alerts.length > 0 ? (
        ...
        <Empty description="暂无成交量提醒"><Button type="primary" onClick={handleAdd}>添加提醒</Button></Empty>
      )}

      <Modal title="添加成交量提醒" open={modalVisible} onOk={handleSubmit} onCancel={() => setModalVisible(false)}>
        <Form form={form} layout="vertical">
          ...
          <Form.Item name="alertType" label="提醒类型" rules={[{ required: true }]}>
            <Select placeholder="选择提醒类型">
              <Select.Option value="VOLUME_SURGE">放量突破</Select.Option>
              <Select.Option value="VOLUME_SHRINK">缩量回调</Select.Option>
            </Select>
          </Form.Item>
          <Form.Item name="volumeThreshold" label="成交量阈值" initialValue={200}>
            <InputNumber min={100} max={1000} addonAfter="%" style={{ width: '100%' }} />
          </Form.Item>
        </Form>
      </Modal>
    </Card>
